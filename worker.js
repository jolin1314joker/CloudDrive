...
const FILE_ICON_BY_EXT = new Map([
  ...['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp', 'ico'].map(ext => [ext, { icon: 'image', color: '#34A853' }]),
  ...['mp4', 'webm', 'ogg', 'avi', 'mov', 'mkv'].map(ext => [ext, { icon: 'movie', color: '#EA4335' }]),
  ...['mp3', 'wav', 'flac', 'aac', 'm4a', 'opus'].map(ext => [ext, { icon: 'audio_file', color: '#FBBC04' }]),
  ...['pdf', 'doc', 'docx'].map(ext => [ext, { icon: 'description', color: '#4285F4' }]),
  ...['xls', 'xlsx', 'csv'].map(ext => [ext, { icon: 'table_chart', color: '#34A853' }]),
  ...['ppt', 'pptx'].map(ext => [ext, { icon: 'slideshow', color: '#FF6D00' }]),
  ...['js', 'ts', 'py', 'java', 'c', 'cpp', 'go', 'rs', 'rb', 'php', 'html', 'css', 'json', 'xml', 'yaml', 'yml'].map(ext => [ext, { icon: 'code', color: '#9C27B0' }]),
  ...['zip', 'tar', 'gz', 'rar', '7z'].map(ext => [ext, { icon: 'folder_zip', color: '#795548' }]),
  ...['txt', 'md', 'log'].map(ext => [ext, { icon: 'article', color: '#607D8B' }])
]);

function getFileIcon(filename) {
  const ext = filename.split('.').pop()?.toLowerCase() || '';
  return FILE_ICON_BY_EXT.get(ext) || { icon: 'insert_drive_file', color: '#5F6368' };
}
...
function normalizeFolderInfo(folder) {
  if (typeof folder === 'string') return { name: folder, size: 0, uploaded: '' };
  return {
    name: folder?.name || '',
    size: Number(folder?.size) || 0,
    uploaded: folder?.uploaded || folder?.updatedAt || ''
  };
}
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  [data-theme="dark"] {
    ...
    --glass-bg: rgba(28,28,32,.55);
    --glass-bg-strong: rgba(28,28,32,.75);
    --glass-border: rgba(255,255,255,.22);
    --glass-shadow: 0 8px 32px rgba(0,0,0,.45), inset 0 1px 0 rgba(255,255,255,.18);
  }
  [data-theme="dark"] .snackbar { background: rgba(48,48,52,.82); color: #E8EAED; backdrop-filter: var(--glass-blur); -webkit-backdrop-filter: var(--glass-blur); }
  [data-theme="dark"] .snackbar-action { color: #8AB4F8; }
  [data-theme="dark"] .login-card { background: rgba(30,30,32,.72); backdrop-filter: var(--glass-blur); -webkit-backdrop-filter: var(--glass-blur); border: 1px solid var(--glass-border); box-shadow: var(--glass-shadow); }
  ...
  body {
    ...
    background:
      radial-gradient(1200px 600px at 12% -10%, rgba(26,115,232,.16), transparent 60%),
      radial-gradient(1000px 520px at 88% -8%, rgba(156,39,176,.14), transparent 60%),
      radial-gradient(900px 700px at 50% 115%, rgba(52,168,83,.12), transparent 60%),
      var(--background);
  ...
  body::before {
    ...
    background:
      radial-gradient(520px 320px at 18% 22%, rgba(66,133,244,.20), transparent 70%),
      radial-gradient(560px 340px at 82% 18%, rgba(187,134,252,.20), transparent 70%),
      radial-gradient(640px 380px at 55% 88%, rgba(52,168,83,.14), transparent 70%);
    filter: saturate(160%);
  }
  body.has-custom-bg {
  ...
  body.has-custom-bg::before {
    background:
      linear-gradient(180deg, rgba(248,249,250,.55), rgba(248,249,250,.72)),
      var(--drive-bg-image) center / cover no-repeat;
    filter: none;
    opacity: 1;
  }
  [data-theme="dark"] body.has-custom-bg::before {
    background:
      linear-gradient(180deg, rgba(18,18,18,.58), rgba(18,18,18,.78)),
      var(--drive-bg-image) center / cover no-repeat;
  }
  header.app-bar, .layout, footer.foot-bar, .login-wrap, .mobile-nav { position: relative; z-index: 1; }
  ...
  ::view-transition-new(root) {
    ...
    /* Prevent stuck view-transition layers from eating all clicks */
  ...
  .file-card:hover { box-shadow: 0 12px 40px rgba(31,38,135,.22); border-color: rgba(255,255,255,.9); transform: translateY(-2px); }
  .file-card.selected { border-color: var(--primary); background: var(--primary-light); box-shadow: 0 0 0 2px var(--primary) inset; }
  .file-card-icon {
  ...
  .progress-item-name { font-size: 13px; display: flex; justify-content: space-between; gap: 12px; align-items: baseline; }
  .progress-item-name > span:last-child { flex-shrink: 0; color: var(--on-surface-variant); font-variant-numeric: tabular-nums; white-space: nowrap; }
  .progress-bar { height: 4px; background: var(--outline); border-radius: 2px; overflow: hidden; }
  .progress-fill { height: 100%; background: var(--primary); border-radius: 2px; transition: width .15s linear; }
  ...
  [data-theme="dark"] .login-wrap { background: rgba(18,18,18,.55); }
  .login-bg-image {
  ...
  @media (max-width: 768px) {
    ...
    .file-grid { grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 6px; }
    .app-bar {
      height: 56px;
      padding: 0 12px;
    ...
    .app-bar-title { font-size: 16px; max-width: 42vw; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .app-bar-actions { gap: 2px; }
    .app-bar-actions .icon-btn { width: 36px; height: 36px; }
    .app-bar-actions .desktop-only { display: none; }
    .login-theme-toggle { top: 16px; right: 16px; }
    .toolbar { gap: 8px; flex-wrap: wrap; }
    .fab, .btn-outlined { height: 36px; padding: 0 14px; font-size: 13px; }
    .action-bar {
      overflow-x: auto;
      flex-wrap: nowrap;
    ...
    .action-btn span:not(.material-icons-round) { display: none; }
    .file-row-name { max-width: 46vw; }
    .file-list th:nth-child(4), .file-list td:nth-child(4) { display: none; }
    ...
    .crop-modal { width: 100%; max-width: 100%; height: 100%; max-height: 100%; border-radius: 0; }
    .crop-stage { min-height: 52vh; }
  }
  ...
  .mobile-nav-btn {
    ...
    color: var(--on-surface-variant); font-size: 11px; font-family: var(--font-body);
  }
  ...
  /* ── Settings / Crop ── */
  .bg-preview {
    width: 100%; aspect-ratio: 16 / 9; border-radius: var(--radius-m);
  ...
  .crop-stage {
    ...
    background: #111; touch-action: none; overflow: hidden;
    user-select: none;
  }
  .crop-stage canvas { display: block; width: 100%; height: 100%; }
  .crop-hint {
    padding: 10px 16px; font-size: 13px; color: var(--on-surface-variant);
  ...
  @keyframes spin { to { transform: rotate(360deg); } }
    ...
    /* ── iOS 26 Liquid Glass + Dynamic Island ── */
  .app-bar, .sidebar {
  ...
  .modal, .preview-modal, .context-menu, .login-card {
  ...
  .modal-overlay.open .modal { animation: glassPop .35s cubic-bezier(.22,1,.36,1); }
  @keyframes glassPop { from { opacity: 0; transform: translateY(24px) scale(.96); } to { opacity: 1; transform: none; } }
  .file-card {
    background: var(--glass-bg) !important;
    backdrop-filter: var(--glass-blur);
    -webkit-backdrop-filter: var(--glass-blur);
  ...
  .file-card.selected { background: var(--primary-light) !important; border-color: var(--primary) !important; }
  @keyframes cardIn { from { opacity: 0; transform: translateY(14px) scale(.97); } to { opacity: 1; transform: none; } }
  .file-card:nth-child(3n) { animation-delay: .04s; }
  .file-card:nth-child(4n) { animation-delay: .07s; }
  .file-card:nth-child(5n) { animation-delay: .1s; }
  .file-list tbody tr { animation: cardIn .3s ease backwards; }
  .foot-bar {
    background: var(--glass-bg) !important;
    backdrop-filter: var(--glass-blur);
    -webkit-backdrop-filter: var(--glass-blur);
  ...
  .snackbar {
    background: rgba(24,24,28,.78) !important;
    backdrop-filter: var(--glass-blur);
    -webkit-backdrop-filter: var(--glass-blur);
  ...
  .selection-bar {
    ...
    transform: translateX(-50%) translateY(120px) scale(.9) !important;
    ...
    box-shadow: 0 12px 40px rgba(0,0,0,.4), inset 0 1px 0 rgba(255,255,255,.25) !important;
    color: var(--island-fg) !important;
    transition: transform .38s cubic-bezier(.34,1.56,.64,1), opacity .25s !important;
    opacity: 0; overflow-x: auto;
    visibility: hidden;
  }
  .selection-bar.open {
  ...
  .selection-bar-count { font-size: 14px !important; font-weight: 600 !important; padding: 0 10px; white-space: nowrap; flex: none !important; }
  .selection-bar .icon-btn { color: #fff !important; width: 42px; height: 42px; flex: none; }
  .selection-bar .icon-btn:hover { background: rgba(255,255,255,.18) !important; }
  .island-divider { width: 1px; height: 24px; background: rgba(255,255,255,.25); margin: 0 4px; flex: none; }
  /* 独立圆形复选框 */
  .row-check {
  ...
  .row-check:hover { border-color: var(--primary); transform: scale(1.12); }
  .row-check.on { background: var(--primary); border-color: var(--primary); color: #fff; }
  .row-check .material-icons-round { font-size: 15px !important; display: none; }
  .row-check.on .material-icons-round { display: inline-flex; }
  .row-check.all { width: 26px; height: 26px; }
  .row-check.all.partial { background: var(--primary); border-color: var(--primary); color: #fff; }
  .row-check.all.partial .material-icons-round { display: inline-flex; }
  .file-card-top { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; }
  .file-card-top .file-card-actions { position: static; opacity: 1; display: flex; gap: 2px; margin-left: auto; }
  .file-card-top .row-check { margin: 2px 0 0 2px; }
  .file-row-checkcell { width: 40px; }
  th.file-row-checkcell { cursor: default; }
  th.file-row-checkcell:hover { color: var(--on-surface-variant); }
  /* 分享短链后缀输入 */
  .suffix-wrap { display: flex; align-items: center; gap: 0; }
  .suffix-prefix {
    padding: 12px 4px 12px 12px; font-size: 14px; color: var(--on-surface-variant);
    border: 1px solid var(--outline); border-right: none; border-radius: 4px 0 0 4px;
    background: rgba(60,64,67,.06); white-space: nowrap;
  }
  .suffix-wrap .text-field { border-radius: 0 4px 4px 0; }
  .suffix-ok { color: var(--success); } .suffix-bad { color: var(--error); }
  @media (max-width: 720px) { .selection-bar { bottom: 14px !important; } }
  [data-theme="dark"] .row-check { background: rgba(255,255,255,.12); border-color: rgba(255,255,255,.45); }
  [data-theme="dark"] .suffix-prefix { background: rgba(255,255,255,.08); border-color: var(--outline); color: var(--on-surface-variant); }
  [data-theme="dark"] .selection-bar { background: rgba(24,24,30,.82) !important; }
...
const RANGED_DOWNLOAD_THRESHOLD = 32 * 1024 * 1024;
const RANGED_DOWNLOAD_CHUNK = 32 * 1024 * 1024;
const RANGED_DOWNLOAD_RETRIES = 3;
const FETCH_BLOB_DOWNLOAD_LIMIT = 200 * 1024 * 1024;
...
function showDownloadProgress(filename, size) {
  ...
  if (els.stats) els.stats.textContent = formatTransferStats(0, size, 0, false);
...
function updateDownloadProgress(filename, downloaded, size, speed) {
  ...
  if (els.name) els.name.textContent = filename;
  if (els.stats) els.stats.textContent = formatTransferStats(downloaded, size, speed, false);
  if (els.fill) els.fill.style.width = (size > 0 ? pct : Math.min(95, downloaded > 0 ? 15 : 0)).toFixed(2) + '%';
  return true;
}
...
function finishDownloadProgress(filename, size, ok) {
  ...
  if (els.name) els.name.textContent = filename;
  if (els.stats) els.stats.textContent = ok ? formatTransferStats(size, size, 0, true) : '下载失败';
...
async function resolveDownloadSize(path, hinted = 0) {
  const hint = Number(hinted) || 0;
  if (hint > 0) return hint;
  try {
    const res = await fetch(downloadUrl(path), { method: 'HEAD' });
    if (res.ok) {
      ...
      if (slash >= 0) {
        const total = Number(String(cr.slice(slash + 1)).trim());
  ...
  try {
    ...
    while (downloaded < size) {
      ...
      while (true) {
        ...
        const speed = speedOf(downloaded, downloaded >= size);
...
async function downloadViaFetch(path, size) {
  const filename = path.split('/').pop() || 'download';
  const downloadId = ++activeDownloadId;
  showDownloadProgress(filename, size);
  try {
    ...
    let total = size || Number(res.headers.get('Content-Length') || 0);
    const reader = res.body.getReader();
    const chunks = [];
    let downloaded = 0;
    const speedOf = trackTransferSpeed();
    while (true) {
      ...
      if (!total) {
        const cr = res.headers.get('Content-Range') || '';
        const slash = cr.lastIndexOf('/');
        if (slash >= 0) total = Number(String(cr.slice(slash + 1)).trim()) || 0;
      }
      const speed = speedOf(downloaded, total > 0 && downloaded >= total);
      if (downloadId === activeDownloadId) updateDownloadProgress(filename, downloaded, total, speed);
    }
    if (!total) total = downloaded;
    ...
    if (downloadId === activeDownloadId) finishDownloadProgress(filename, total, true);
  } catch (err) {
    console.error(err);
    if (downloadId === activeDownloadId) finishDownloadProgress(filename, size, false);
    showSnackbar('下载失败');
...
async function startDownload(path, size = 0) {
  const filename = path.split('/').pop() || 'download';
  let total = Number(size) || 0;
  if (total <= 0) total = await resolveDownloadSize(path, 0);

  // Large files: stream to disk via File System Access API (avoids memory blowup)
  if (total >= RANGED_DOWNLOAD_THRESHOLD && 'showSaveFilePicker' in window) {
    downloadInRanges(path, total);
    return;
  }
  // Typical files: fetch with in-page progress showing total size + speed
  if (total <= FETCH_BLOB_DOWNLOAD_LIMIT) {
    downloadViaFetch(path, total);
    return;
  }
  if (total > 0) showSnackbar('开始下载 ' + filename + '（' + formatSize(total) + '）');
    ...
    fetch(url)
      ...
      .then(function(text) {
        ...
        if (text.length > PREVIEW_TEXT_MAX) {
          // NOTE: this script lives inside a template literal — use \\n so the browser receives \n
          el.textContent = text.slice(0, PREVIEW_TEXT_MAX) + '\\n\\n… 文件过大，仅显示前 2MB，请下载查看完整内容。';
        } else {
          el.textContent = text;
        }
      })
      .catch(function(err) {
        var el = document.getElementById('previewTextContent');
        if (!el) return;
        el.textContent = err && err.message === 'too large'
          ? '文件过大，无法在线预览，请下载查看。'
          : '文件加载失败，请尝试下载查看。';
      });
...
// ── Selection ──
// ── File Click: 单击选中，双击预览（不可预览则下载）；复选框也可选中
...
// ── Folder Click: 单击选中，双击进入；Ctrl/Meta 单击选中；勾选/按钮不触发跳转
...
function toggleSelect(name, el) {
  ...
  // Sync both grid + list DOM so view switching cannot desync checkboxes
  syncSelectionDom();
  updateSelectionBar();
  updateActionBar();
}
...
function toggleSelectAll() {
  ...
  else items.forEach(name => selectedFiles.add(name));
  syncSelectionDom();
  updateSelectionBar();
  updateActionBar();
}
...
function updateSelectAllState() {
  ...
  const allBtns = document.querySelectorAll('.row-check.all');
  allBtns.forEach(btn => {
...
function clearSelection() {
  ...
  document.querySelectorAll('.row-check.on').forEach(btn => btn.classList.remove('on'));
  updateSelectionBar();
  updateActionBar();
}
...
function setPasteButtonsDisabled(disabled) {
  document.querySelectorAll('#pasteBtn, #pasteBtnIsland').forEach(btn => {
...
function renameSelected() {
  ...
  fetch('/api/rename', {
    ...
    showSnackbar(await fetchErrorMessage(r, '重命名失败'));
  }).catch(() => showSnackbar('重命名失败'));
}
...
function renderStorageDetails(nodes) {
  ...
  const data = window.storageInfoData || {};
  const logical = Number(data.logicalUsed || data.used || 0);
  const physical = Number(data.physicalUsed || 0);
  let orphanHint = '';
  if (physical > logical + 1024 * 1024) {
    orphanHint = '<div class="storage-node-meta" style="margin-bottom:8px">文件占用 ' + formatSize(logical) +
      '，节点物理占用 ' + formatSize(physical) +
      '。差额可能来自未清理的孤儿对象，可使用「孤儿清理」。</div>';
  }
  list.innerHTML = orphanHint + nodes.map(node => {
...
function deleteSelected() {
  ...
  fetch('/api/delete-batch', {
    ...
    if (r.ok) {
      showSnackbar('已删除 ' + selectedFiles.size + ' 个文件');
      location.reload();
      return;
    }
    showSnackbar(await fetchErrorMessage(r, '删除失败'));
...
function resetShareForm() {
  ...
  const slugHint = document.getElementById('shareSlugHint');
  if (password) {
    password.value = '';
  ...
  if (slugHint) {
    slugHint.textContent = '留空自动生成安全 UUID；支持 2-48 位字母 / 数字 / _ / -';
    slugHint.classList.remove('suffix-ok', 'suffix-bad');
  }
...
async function submitShareForm() {
  ...
  const slugRaw = document.getElementById('shareSlugInput')?.value || '';
  ...
  if (slugRaw.trim() && !/^[a-zA-Z0-9_-]{2,48}$/.test(slugRaw.trim())) {
    showSnackbar('短链后缀仅支持 2-48 位字母、数字、下划线或中划线');
    return;
  }
  ...
  try {
    ...
    if (slugRaw.trim()) body.customId = slugRaw.trim();
    if (shareEditId) {
      body.id = shareEditId;
    ...
    if (slugInput) slugInput.value = data.share.id;
...
async function handleSlugInput() {
  const input = document.getElementById('shareSlugInput');
  const hint = document.getElementById('shareSlugHint');
  if (!input || !hint) return;
  const value = input.value.trim();
  hint.classList.remove('suffix-ok', 'suffix-bad');
  const seq = ++slugCheckSeq;
  if (!value) {
    hint.textContent = '留空自动生成安全 UUID；支持 2-48 位字母 / 数字 / _ / -';
    return;
  }
  if (!/^[a-zA-Z0-9_-]{2,48}$/.test(value)) {
    hint.textContent = '格式不正确：需 2-48 位字母、数字、下划线或中划线';
    hint.classList.add('suffix-bad');
    return;
  }
  hint.textContent = '检查可用性中...';
  clearTimeout(slugCheckTimer);
  slugCheckTimer = setTimeout(async () => {
    try {
      const res = await fetch('/api/shares/slug-check?slug=' + encodeURIComponent(value));
      const data = await res.json().catch(() => ({}));
      if (seq !== slugCheckSeq) return;
      if (res.ok && data.available) {
        hint.textContent = '短链可用：' + location.origin + '/s/' + value;
        hint.classList.add('suffix-ok');
      } else {
        hint.textContent = data.error || '该后缀已被占用或是保留词，请换一个';
        hint.classList.add('suffix-bad');
      }
    } catch {
      if (seq === slugCheckSeq) {
        hint.textContent = '无法检查后缀可用性，请稍后重试';
        hint.classList.add('suffix-bad');
      }
    }
  }, 350);
...
function editShareRecord(id) {
  ...
  const slugInput = document.getElementById('shareSlugInput');
  const slugHint = document.getElementById('shareSlugHint');
  if (slugInput) slugInput.value = share.id || '';
  if (slugHint) {
    slugHint.textContent = '当前后缀：/s/' + (share.id || '') + '，可修改后保存';
    slugHint.classList.remove('suffix-ok', 'suffix-bad');
  }
  if (clearPassword) clearPassword.checked = false;
...
function ctxDelete() {
  const items = ctxItems();
  if (!items.length) return;
  if (!confirm('确定删除选中的 ' + items.length + ' 项？')) return;
  const paths = items.map(name => currentPath ? currentPath + '/' + name : name);
  fetch('/api/delete-batch', {
    method: 'POST',
    headers: jsonHeaders(),
    body: JSON.stringify({ paths })
...
function createUploadProgressUpdater(fileSize, fill, pctSpan) {
  const speedOf = trackTransferSpeed();
  return (loaded, done) => {
    const total = Number(fileSize) || 0;
    const current = Math.max(0, Number(loaded) || 0);
    const speed = speedOf(current, !!done || (total > 0 && current >= total));
    if (done) {
      fill.style.width = '100%';
      pctSpan.textContent = formatTransferStats(total || current, total || current, speed, true);
      return;
    }
    const pct = total > 0 ? Math.min(99, Math.round(current / total * 100)) : 0;
    fill.style.width = (total > 0 ? pct : Math.min(95, current > 0 ? 15 : 0)) + '%';
    pctSpan.textContent = formatTransferStats(current, total, speed, false);
  };
}
...
function uploadFiles(files) {
  ...
  const tasks = [...files].map(file => {
    ...
    const pctSpan = document.createElement('span'); pctSpan.textContent = formatTransferStats(0, file.size, 0, false);
...
function uploadSingleFile(file, path, fill, pctSpan) {
  ...
  // 优先分布式；仅在初始化失败或分片尚未传完时回退到主 R2，避免传完又从头开始
  return uploadDistributed(file, path, fill, pctSpan)
    .catch(err => {
      if (err && err.fallback === true) {
        console.warn('[分布式上传失败，回退到主 R2 分片上传]', err?.message || err);
        showSnackbar('分布式存储不可用，改用主存储重试（需重新上传）', '', null);
        fill.classList.remove('done', 'error');
        fill.style.width = '0%';
        pctSpan.textContent = formatTransferStats(0, file.size, 0, false);
...
function uploadDirect(file, path, fill, pctSpan) {
  const update = createUploadProgressUpdater(file.size, fill, pctSpan);
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '/api/upload?path=' + encodeURIComponent(path));
    xhr.setRequestHeader('X-R2Drive-CSRF', 'same-origin');
    xhr.upload.onprogress = e => {
      if (e.lengthComputable) update(e.loaded, false);
      else update(e.loaded, false);
    };
    xhr.onload = () => {
      if (xhr.status === 200) {
        fill.classList.add('done');
        update(file.size, true);
        pctSpan.textContent = '✓ ' + formatSize(file.size);
        resolve();
      } else {
...
async function uploadMultipart(file, path, fill, pctSpan) {
  ...
  try {
    ...
    const updateProgress = () => {
      let loaded = 0;
      for (let i = 0; i < totalParts; i++) loaded += partProgress[i];
      update(loaded, false);
    };
    ...
    const workers = Array.from({ length: Math.min(UPLOAD_CONCURRENCY, totalParts) }, async () => {
      while (true) {
        ...
        partSizes[partNumber - 1] = chunk.size;
...
async function uploadDistributed(file, path, fill, pctSpan) {
  ...
  try {
    ...
    if (!initRes.ok) {
      ...
      const err = new Error(errMsg + (initRes.status === 409 ? '（文件过小）' : ''));
      err.fallback = true;
      throw err;
    }
    ...
    if (err && err.fallback !== false) err.fallback = true;
    throw err;
  }
  ...
  const partProgress = new Array(totalParts).fill(0);
  const partDone = new Array(totalParts).fill(false);
  const UPLOAD_CONCURRENCY = 3;
  try {
    const updateProgress = () => {
      let loaded = 0;
    ...
    // 分片已全部落地：进入确认阶段（此时进度条停在 ~99%，避免误以为失败）
    fill.style.width = '99%';
    pctSpan.textContent = '99% · 正在确认分片并完成上传…';
    ...
    const tryComplete = async (attemptLabel) => {
      if (attemptLabel) pctSpan.textContent = '99% · ' + attemptLabel;
      ...
      if (!completeRes.ok) {
        throw new Error(await fetchErrorMessage(completeRes, '确认分片失败'));
      }
      finishOk();
    };
    ...
    for (let attempt = 0; attempt < 6; attempt++) {
      try {
        await tryComplete(attempt === 0 ? '正在确认分片并完成上传…' : ('正在重试确认 (' + attempt + '/5)…'));
        return;
      } catch (err) {
        lastCompleteError = err;
        await new Promise(resolve => setTimeout(resolve, 1000 * (attempt + 1)));
      }
    }

    // 保留 session，提供一键重试确认，绝不自动整文件重传
    fill.classList.add('error');
    const errText = (lastCompleteError && lastCompleteError.message) ? String(lastCompleteError.message).slice(0, 80) : '确认分片失败';
    pctSpan.textContent = '';
    const wrap = document.createElement('span');
    wrap.style.cssText = 'display:inline-flex;align-items:center;gap:8px;flex-wrap:wrap';
    const msg = document.createElement('span');
    msg.style.color = 'var(--error)';
    msg.textContent = '确认未完成：' + errText;
    const retryBtn = document.createElement('button');
    retryBtn.type = 'button';
    retryBtn.className = 'btn-outlined';
    retryBtn.style.cssText = 'height:28px;padding:0 10px;font-size:12px';
    retryBtn.textContent = '重试确认';
    wrap.append(msg, retryBtn);
    pctSpan.appendChild(wrap);
    showSnackbar('分片已传完，确认暂未成功。可点「重试确认」，无需重新上传。', '', null);

    await new Promise((resolve, reject) => {
      retryBtn.onclick = async () => {
        retryBtn.disabled = true;
        fill.classList.remove('error');
        pctSpan.textContent = '99% · 正在重试确认…';
        try {
          await tryComplete('正在重试确认…');
          showSnackbar('上传已完成', '刷新', () => location.reload());
          resolve();
        } catch (err) {
          fill.classList.add('error');
          pctSpan.textContent = '';
          msg.textContent = '确认未完成：' + String(err.message || '确认失败').slice(0, 80);
          pctSpan.appendChild(wrap);
          retryBtn.disabled = false;
          showSnackbar('确认仍失败：' + (err.message || '请稍后再试'));
        }
      };
    });
    return;
  } catch (err) {
    // 分片已上传成功时不要 abort，否则会删掉已传数据并迫使整文件重传
    if (sessionId && !partsUploaded) {
      ...
      if (err && err.fallback !== false) err.fallback = true;
    } else if (partsUploaded) {
      if (err) err.fallback = false;
    }
    throw err;
  }
}
...
function uploadDistributedPart(partInfo, chunk, onProgress) {
  ...
  const runOnce = () => new Promise((resolve, reject) => {
    ...
    xhr.ontimeout = () => reject(new Error('分片 ' + (partInfo.partNumber || '?') + ' 超时'));
    xhr.send(chunk);
  });
...
function viewNodeR2Files(nodeId, nodeName) {
  ...
  if (r2fileTitle) r2fileTitle.textContent = 'R2 文件 — ' + String(nodeName || '');
...
const DRIVE_BG_ASPECT = 16 / 9;
const DRIVE_BG_ASPECT_TOLERANCE = 0.03;
let cropState = null;

function parseCssUrl(raw) {
  const t = String(raw || '').trim();
  if (!t.toLowerCase().startsWith('url(') || !t.endsWith(')')) return '';
  let inner = t.slice(4, -1).trim();
...
function refreshBgPreview() {
  ...
  if (!preview) return;
  const src = parseCssUrl(getComputedStyle(document.documentElement).getPropertyValue('--drive-bg-image'));
  if (src && document.body.classList.contains('has-custom-bg')) {
    preview.classList.remove('empty');
    preview.style.backgroundImage = 'url(' + JSON.stringify(src).slice(1, -1) + ')';
    preview.textContent = '';
  } else {
    preview.classList.add('empty');
    preview.style.backgroundImage = '';
    preview.textContent = '尚未设置自定义背景';
  }
}
...
async function loadDriveBackground() {
  try {
    ...
    if (!res.ok) return;
    const data = await res.json();
    if (data.driveBackgroundUrl) applyDriveBackgroundUrl(data.driveBackgroundUrl + (data.driveBackgroundUrl.includes('?') ? '&' : '?') + 't=' + Date.now());
    else applyDriveBackgroundUrl('');
  } catch (_) {}
}
...
function handleBgFileInput(e) {
  ...
  reader.onload = () => {
    ...
    img.onload = () => {
      ...
      if (Math.abs(ratio - DRIVE_BG_ASPECT) <= DRIVE_BG_ASPECT_TOLERANCE) {
        canvasFromImage(img, img.naturalWidth, img.naturalHeight, 0, 0, img.naturalWidth, img.naturalHeight)
          .then(blob => uploadDriveBackground(blob))
          .catch(err => showSnackbar('处理图片失败：' + (err.message || '未知错误')));
        return;
      }
      openCropper(img);
    };
    img.onerror = () => showSnackbar('图片加载失败');
    img.src = reader.result;
  };
  reader.onerror = () => showSnackbar('读取图片失败');
  reader.readAsDataURL(file);
}

function canvasFromImage(img, outW, outH, sx, sy, sw, sh) {
  return new Promise((resolve, reject) => {
    const maxW = 1920;
    let tw = outW, th = outH;
...
function openCropper(img) {
  ...
  const layout = () => {
    ...
    const fit = Math.min(rect.width / img.naturalWidth, rect.height / img.naturalHeight) * 0.92;
    state.scale = fit;
    state.offsetX = (rect.width - img.naturalWidth * fit) / 2;
    state.offsetY = (rect.height - img.naturalHeight * fit) / 2;
    const maxCropW = Math.min(rect.width * 0.88, img.naturalWidth * fit);
    const maxCropH = maxCropW / DRIVE_BG_ASPECT;
    let cropH = Math.min(maxCropH, rect.height * 0.78, img.naturalHeight * fit);
    let cropW = cropH * DRIVE_BG_ASPECT;
    if (cropW > maxCropW) { cropW = maxCropW; cropH = cropW / DRIVE_BG_ASPECT; }
    state.cropW = cropW;
    state.cropH = cropH;
    state.cropX = (rect.width - cropW) / 2;
    state.cropY = (rect.height - cropH) / 2;
    drawCropper();
  };
...
function drawCropper() {
  const canvas = document.getElementById('cropCanvas');
  const stage = document.getElementById('cropStage');
  if (!canvas || !stage || !cropState) return;
  const ctx = canvas.getContext('2d');
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  const rect = stage.getBoundingClientRect();
  const s = cropState;
  ...
  ctx.drawImage(s.img, s.offsetX, s.offsetY, s.img.naturalWidth * s.scale, s.img.naturalHeight * s.scale);
  ...
  ctx.lineWidth = 1;
  for (let i = 1; i <= 2; i++) {
    const gx = s.cropX + s.cropW * i / 3;
    const gy = s.cropY + s.cropH * i / 3;
    ctx.beginPath(); ctx.moveTo(gx, s.cropY); ctx.lineTo(gx, s.cropY + s.cropH); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(s.cropX, gy); ctx.lineTo(s.cropX + s.cropW, gy); ctx.stroke();
  }
  // corners
  const hs = 14;
  ctx.strokeStyle = '#fff';
  ...
  const corners = [
    [s.cropX, s.cropY, 1, 1],
    [s.cropX + s.cropW, s.cropY, -1, 1],
    [s.cropX, s.cropY + s.cropH, 1, -1],
    [s.cropX + s.cropW, s.cropY + s.cropH, -1, -1]
  ];
...
function clampCropBox() {
  const stage = document.getElementById('cropStage');
  if (!stage || !cropState) return;
  const rect = stage.getBoundingClientRect();
  const s = cropState;
  s.cropW = Math.max(80, Math.min(s.cropW, rect.width * 0.95));
  s.cropH = s.cropW / DRIVE_BG_ASPECT;
  if (s.cropH > rect.height * 0.92) {
    s.cropH = rect.height * 0.92;
    s.cropW = s.cropH * DRIVE_BG_ASPECT;
  }
  s.cropX = Math.max(0, Math.min(s.cropX, rect.width - s.cropW));
  s.cropY = Math.max(0, Math.min(s.cropY, rect.height - s.cropH));
}
...
function hitCropHandle(x, y) {
  ...
  const hs = 22;
  const points = [
    { id: 'nw', x: s.cropX, y: s.cropY },
    { id: 'ne', x: s.cropX + s.cropW, y: s.cropY },
    { id: 'sw', x: s.cropX, y: s.cropY + s.cropH },
    { id: 'se', x: s.cropX + s.cropW, y: s.cropY + s.cropH }
  ];
  for (const p of points) {
    if (Math.abs(x - p.x) <= hs && Math.abs(y - p.y) <= hs) return p.id;
  }
  if (x >= s.cropX && x <= s.cropX + s.cropW && y >= s.cropY && y <= s.cropY + s.cropH) return 'move';
  return 'pan';
}
...
function onCropPointerDown(e) {
  ...
  cropState.pointers.set(e.pointerId, { x: e.clientX - rect.left, y: e.clientY - rect.top });
  if (cropState.pointers.size === 2) {
    const pts = [...cropState.pointers.values()];
    const dx = pts[0].x - pts[1].x, dy = pts[0].y - pts[1].y;
    cropState.pinchStartDist = Math.hypot(dx, dy) || 1;
    cropState.pinchStartScale = cropState.scale;
    cropState.dragging = { mode: 'pinch' };
    return;
  }
  const x = e.clientX - rect.left, y = e.clientY - rect.top;
  const mode = hitCropHandle(x, y);
  cropState.dragging = { mode, startX: x, startY: y, ox: cropState.offsetX, oy: cropState.offsetY, cx: cropState.cropX, cy: cropState.cropY, cw: cropState.cropW, ch: cropState.cropH };
}
...
function onCropPointerMove(e) {
  ...
  if (cropState.dragging.mode === 'pinch' && cropState.pointers.size === 2) {
    const pts = [...cropState.pointers.values()];
    const dist = Math.hypot(pts[0].x - pts[1].x, pts[0].y - pts[1].y) || 1;
    cropState.scale = Math.max(0.05, Math.min(8, cropState.pinchStartScale * (dist / cropState.pinchStartDist)));
    drawCropper();
    return;
  }
  const x = e.clientX - rect.left, y = e.clientY - rect.top;
  ...
  const s = cropState;
  if (d.mode === 'move') {
    s.cropX = d.cx + dx; s.cropY = d.cy + dy; clampCropBox();
  } else if (d.mode === 'pan') {
    s.offsetX = d.ox + dx; s.offsetY = d.oy + dy;
  } else if (d.mode === 'se' || d.mode === 'ne' || d.mode === 'sw' || d.mode === 'nw') {
    let w = d.cw, h = d.ch, cx = d.cx, cy = d.cy;
    if (d.mode === 'se') { w = d.cw + dx; }
    if (d.mode === 'ne') { w = d.cw + dx; h = w / DRIVE_BG_ASPECT; cy = d.cy + d.ch - h; }
    if (d.mode === 'sw') { w = d.cw - dx; cx = d.cx + dx; }
    if (d.mode === 'nw') { w = d.cw - dx; cx = d.cx + dx; h = w / DRIVE_BG_ASPECT; cy = d.cy + d.ch - h; }
    if (d.mode === 'se' || d.mode === 'sw') h = w / DRIVE_BG_ASPECT;
    if (w < 80) { w = 80; h = w / DRIVE_BG_ASPECT; if (d.mode === 'nw' || d.mode === 'sw') cx = d.cx + d.cw - w; if (d.mode === 'nw' || d.mode === 'ne') cy = d.cy + d.ch - h; }
    s.cropW = w; s.cropH = h; s.cropX = cx; s.cropY = cy; clampCropBox();
  }
  drawCropper();
}
...
function onCropPointerUp(e) {
  ...
  if (cropState.pointers.size < 2 && cropState.dragging?.mode === 'pinch') cropState.dragging = null;
  if (!cropState.pointers.size) cropState.dragging = null;
}

async function confirmCrop() {
  if (!cropState) return;
  const s = cropState;
  const sx = (s.cropX - s.offsetX) / s.scale;
  const sy = (s.cropY - s.offsetY) / s.scale;
  const sw = s.cropW / s.scale;
  const sh = s.cropH / s.scale;
  const x = Math.max(0, Math.min(s.img.naturalWidth, sx));
  const y = Math.max(0, Math.min(s.img.naturalHeight, sy));
  const w = Math.max(1, Math.min(s.img.naturalWidth - x, sw));
  const h = Math.max(1, Math.min(s.img.naturalHeight - y, sh));
  try {
    const blob = await canvasFromImage(s.img, Math.round(w), Math.round(h), x, y, w, h);
    closeCropper();
    await uploadDriveBackground(blob);
  } catch (err) {
    showSnackbar('裁剪失败：' + (err.message || '未知错误'));
  }
}

async function uploadDriveBackground(blob) {
  showSnackbar('正在上传背景...');
  try {
    const res = await fetch('/api/ui-background', {
      method: 'PUT',
      headers: Object.assign({ 'Content-Type': 'image/jpeg' }, CSRF_HEADER),
      body: blob
    });
    if (!res.ok) throw new Error(await fetchErrorMessage(res, '上传失败'));
    const data = await res.json().catch(() => ({}));
    const url = data.url || '/api/ui-background';
    applyDriveBackgroundUrl(url + (url.includes('?') ? '&' : '?') + 't=' + Date.now());
    refreshBgPreview();
    showSnackbar('背景已更新');
  } catch (err) {
    showSnackbar('设置背景失败：' + (err.message || '未知错误'));
  }
}

async function clearDriveBackground() {
  if (!confirm('确定清除自定义背景？')) return;
  try {
    ...
    applyDriveBackgroundUrl('');
    refreshBgPreview();
    showSnackbar('已恢复默认背景');
  } catch (err) {
    showSnackbar('清除失败：' + (err.message || '未知错误'));
  }
}
...
<header class="app-bar">
  <a class="app-bar-logo" href="${sharedBase}">
    ${renderLogoIcon(cloudIconUrl)}
    <span class="app-bar-title">${escapeHtml(siteTitle)} - 共享</span>
  </a>
...
<div class="layout">
  ...
  <main class="main">
    ...
    <div id="fileGrid" class="file-grid">
      ${folders.map(folder => {
        const info = normalizeFolderInfo(folder);
        const href = sharedBase + '/?path=' + encodeURIComponent(currentPath ? currentPath + '/' + info.name : info.name);
        return `<div class="file-card" onclick="${jsAttr(`location.href=${jsString(href)}`)}">
          ...
          <div class="file-card-name">${escapeHtml(info.name)}</div>
          <div class="file-card-meta">
            <span>${formatSize(info.size)}</span>
            <span>${formatDate(info.uploaded)}</span>
          </div>
        </div>`;
      }).join('')}
            ${files.map(file => {
          ...
          ${folders.map(folder => {
            ...
            return `<tr>
              <td><div class="file-row-icon">
                ...
                <span class="file-row-name" onclick="${jsAttr(`location.href=${jsString(href)}`)}">${escapeHtml(info.name)}</span>
              </div></td>
              <td class="file-row-meta">${formatSize(info.size)}</td>
              <td class="file-row-meta">${formatDate(info.uploaded)}</td>
              <td></td>
            </tr>`;
          }).join('')}
          ${files.map(file => {
...
<div class="layout">
  ...
  <main class="main">
    ...
    <div id="fileGrid" class="file-grid">
      ${folders.map(folder => {
        const info = normalizeFolderInfo(folder);
        const href = base + '/?path=' + encodeURIComponent(currentPath ? currentPath + '/' + info.name : info.name);
        return `<div class="file-card" onclick="${jsAttr(`location.href=${jsString(href)}`)}">
          <div class="file-card-icon" style="background:#FFF8E1">
            <span class="material-icons-round" style="color:#F9AB00;font-size:32px">folder</span>
          </div>
        ...
        <tbody>
          ${folders.map(folder => {
            ...
            return `<tr onclick="${jsAttr(`location.href=${jsString(href)}`)}">
              <td><div class="file-row-icon">
                <span class="material-icons-round" style="color:#F9AB00;font-size:22px">folder</span>
                <span class="file-row-name">${escapeHtml(info.name)}</span>
              </div></td>
              <td class="file-row-meta">${formatSize(info.size)}</td>
              <td class="file-row-meta">${formatDate(info.uploaded)}</td>
              <td></td>
...
function renderDrivePage(folders, files, currentPath, siteTitle, cloudIconUrl = '') {
  ...
  const breadcrumb = `<nav class="breadcrumb">
    ...
    return `<div class="file-card" data-name="${escapeAttr(name)}" data-size="${info.size}" data-date="${escapeAttr(info.uploaded || '')}"
        onclick="${jsAttr(`handleFolderClick(event, ${jsString(name)}, ${jsString(href)})`)}"
        oncontextmenu="${jsAttr(`showCtxMenu(event, ${jsString(name)})`)}">
      <div class="file-card-top">
        <button class="row-check" title="选中" onclick="${jsAttr(`event.stopPropagation();toggleSelect(${jsString(name)}, this)`)}">
    ...
    return `<div class="file-card" data-name="${escapeAttr(file.name)}" data-size="${size}" onclick="${jsAttr(`handleFileClick(event, ${jsString(file.name)})`)}"
      ...
      <div class="file-card-top">
        <button class="row-check" title="选中" onclick="${jsAttr(`event.stopPropagation();toggleSelect(${jsString(file.name)}, this)`)}">
          <span class="material-icons-round">check</span>
        </button>
        <div class="file-card-actions">
    ...
    const name = info.name;
    const href = '/?path=' + encodeURIComponent(currentPath ? currentPath + '/' + name : name);
    return `<tr data-name="${escapeAttr(name)}" data-size="${info.size}" data-date="${escapeAttr(info.uploaded || '')}" onclick="${jsAttr(`handleFolderClick(event, ${jsString(name)}, ${jsString(href)})`)}">
      <td class="file-row-checkcell"><button class="row-check" title="选中" onclick="${jsAttr(`event.stopPropagation();toggleSelect(${jsString(name)}, this)`)}"><span class="material-icons-round">check</span></button></td>
      <td><div class="file-row-icon">
        <span class="material-icons-round" style="color:#F9AB00;font-size:22px">folder</span>
    ...
    return `<tr data-name="${escapeAttr(file.name)}" data-size="${size}" data-date="${escapeAttr(file.uploaded || '')}" onclick="${jsAttr(`handleFileClick(event, ${jsString(file.name)})`)}">
      <td class="file-row-checkcell"><button class="row-check" title="选中" onclick="${jsAttr(`event.stopPropagation();toggleSelect(${jsString(file.name)}, this)`)}"><span class="material-icons-round">check</span></button></td>
      <td><div class="file-row-icon">
        <span class="material-icons-round" style="color:${color};font-size:22px">${icon}</span>
        <span class="file-row-name">${escapeHtml(file.name)}</span>
      </div></td>
...
<header class="app-bar">
  ...
  <div class="app-bar-spacer"></div>
    ...
    <button class="icon-btn desktop-only" title="存储节点" onclick="openStorageNodes()">
      <span class="material-icons-round">hub</span>
    </button>
    <button class="icon-btn desktop-only" title="刷新" onclick="location.reload()">
...
<div class="layout">
  <nav class="sidebar">
      ...
      <button type="button" class="sidebar-item" onclick="openUpload()">
        <span class="material-icons-round">cloud_upload</span> 上传文件
      </button>
      <button type="button" class="sidebar-item" onclick="openStorageNodes()">
        <span class="material-icons-round">hub</span> 存储节点
      </button>
      <button type="button" class="sidebar-item" onclick="openOrphanCleanup()">
        <span class="material-icons-round">cleaning_services</span> 孤儿清理
      </button>
    </div>
    <div class="sidebar-divider"></div>
    <div class="sidebar-section">
      ...
      <a class="sidebar-item" href="/?path=${encodeURIComponent('shared')}">
        <span class="material-icons-round">folder_shared</span> 共享文件夹
      </a>
    </div>
  ...
  <main class="main">
    ...
    <div class="toolbar">
      ...
      <button class="row-check all" title="全选 / 取消全选" onclick="toggleSelectAll()">
        <span class="material-icons-round">check</span>
      </button>
      <div class="toolbar-right">
        <div class="view-toggle">
    ...
    <div id="fileList" style="display:none">
      <table class="file-list">
        <thead>
          <tr>
            <th class="file-row-checkcell" title="全选"><button class="row-check all" title="全选 / 取消全选" onclick="toggleSelectAll()"><span class="material-icons-round">check</span></button></th>
...
<div class="selection-bar" id="selectionBar">
  ...
  <button class="icon-btn" onclick="previewSelected()" title="预览">
    <span class="material-icons-round">visibility</span>
  </button>
  <button class="icon-btn" onclick="renameSelected()" title="重命名">
    <span class="material-icons-round">drive_file_rename_outline</span>
  </button>
  <button class="icon-btn" onclick="shareSelected()" title="分享">
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="share-form-grid">
        ...
        <div class="full">
          <label class="field-label" for="shareSlugInput">自定义短链后缀</label>
          <div class="suffix-wrap">
            <span class="suffix-prefix">/s/</span>
            <input class="text-field" id="shareSlugInput" type="text" maxlength="48" placeholder="例如: my-photo-2026"
              autocomplete="off" spellcheck="false" oninput="handleSlugInput()">
          </div>
          <div class="share-hint" id="shareSlugHint">留空自动生成安全 UUID；支持 2-48 位字母 / 数字 / _ / -</div>
        </div>
      </div>
...
<div class="modal-overlay" id="settingsModal" onclick="if(event.target===this)closeSettings()">
  <div class="modal">
    ...
    <div class="modal-body">
      <label class="field-label">自定义背景</label>
      <div class="bg-preview empty" id="bgPreview">尚未设置自定义背景</div>
      <p class="share-hint" style="margin-bottom:12px">建议使用 16:9 图片。其他比例会打开裁剪窗口，可自由拖动与缩放裁剪框。</p>
      <div class="settings-actions">
        <button class="fab" style="box-shadow:none" onclick="pickBackgroundImage()">
          <span class="material-icons-round">add_photo_alternate</span> 上传背景
        </button>
        <button class="btn-outlined" onclick="clearDriveBackground()">
          <span class="material-icons-round">hide_image</span> 清除背景
        </button>
      </div>
...
<div class="crop-overlay" id="cropOverlay">
  <div class="crop-modal">
    <div class="modal-header">
      <span class="material-icons-round" style="color:var(--primary)">crop_16_9</span>
      <span class="modal-title">裁剪背景（16:9）</span>
    </div>
    <div class="crop-stage" id="cropStage">
      <canvas id="cropCanvas"></canvas>
    </div>
    <div class="crop-hint">拖动裁剪框移动，拖角缩放；单指空白处平移图片，双指缩放。</div>
    <div class="crop-footer">
      <button class="btn-outlined" onclick="closeCropper()">取消</button>
      <button class="fab" style="box-shadow:none" onclick="confirmCrop()">
        <span class="material-icons-round">check</span> 应用
      </button>
    </div>
  </div>
</div>
...
<nav class="mobile-nav" aria-label="手机导航">
  <button type="button" class="mobile-nav-btn active" onclick="location.href='/'">
    <span class="material-icons-round">cloud</span>云盘
  </button>
  <button type="button" class="mobile-nav-btn" onclick="openUpload()">
    <span class="material-icons-round">upload</span>上传
  </button>
  <button type="button" class="mobile-nav-btn" onclick="openSettings()">
    <span class="material-icons-round">settings</span>设置
  </button>
  <button type="button" class="mobile-nav-btn" onclick="openStorageNodes()">
    <span class="material-icons-round">hub</span>节点
  </button>
</nav>
`, siteTitle);
}
...
const UI_SETTINGS_KV_KEY = 'r2drive:ui:settings';
const UI_DRIVE_BG_R2_KEY = 'r2drive_ui_drive_background.jpg';
...
const R2_USAGE_CACHE_TTL_MS = 60 * 1000;
const LOGICAL_USAGE_CACHE_TTL_MS = 30 * 1000;
const r2UsageCache = new WeakMap();
let logicalUsageCache = null;

function invalidateStorageUsageCaches(R2) {
  logicalUsageCache = null;
  if (R2) r2UsageCache.delete(R2);
}
...
async function verifyAccessPassword(password, expected) {
  if (!expected) return true;
  const [left, right] = await Promise.all([
    sha256Hex(String(password ?? '')),
    sha256Hex(String(expected))
  ]);
...
async function calculateR2Usage(R2, options = {}) {
  if (!R2) return 0;
  const force = options.force === true;
  const cached = r2UsageCache.get(R2);
  if (!force && cached && (Date.now() - cached.at) < R2_USAGE_CACHE_TTL_MS) {
    return cached.used;
  }
  ...
  let safety = 0;
  do {
    // omit customMetadata — listing size only, cheaper on large buckets
  ...
  r2UsageCache.set(R2, { used: totalUsed, at: Date.now() });
  return totalUsed;
}

/** Sum of virtual file sizes (matches folder/file UI). Ignores R2 orphans / stale estimates. */
async function calculateLogicalStorageUsage(env, options = {}) {
  const force = options.force === true;
  if (!force && logicalUsageCache && (Date.now() - logicalUsageCache.at) < LOGICAL_USAGE_CACHE_TTL_MS) {
    return logicalUsageCache.used;
  }
  const files = await listAllFileEntries(env, '');
  let used = 0;
  for (const file of files) used += Math.max(0, Number(file.size) || 0);
  logicalUsageCache = { used, at: Date.now() };
  return used;
}
...
function d1KvStore(DB) {
  return {
    ...
    async list({ prefix = '', cursor = '', limit = 1000 } = {}) {
      ...
      // Keyset pagination: cursor is the last key from the previous page (avoids OFFSET scans)
      const afterKey = cursor ? String(cursor) : '';
      const result = afterKey
        ? await DB.prepare(`
            SELECT "key" AS name
            FROM ${D1_KV_TABLE}
            ...
            ORDER BY "key"
            LIMIT ?
          `).bind(afterKey, upper, now, pageSize + 1).all()
        : await DB.prepare(`
            SELECT "key" AS name
            FROM ${D1_KV_TABLE}
            WHERE "key" >= ? AND "key" < ? AND (expires_at IS NULL OR expires_at > ?)
            ORDER BY "key"
            LIMIT ?
          `).bind(prefix, upper, now, pageSize + 1).all();
      const rows = result.results || [];
      const page = rows.slice(0, pageSize);
      return {
        keys: page.map(row => ({ name: row.name })),
        cursor: rows.length > pageSize ? page[page.length - 1]?.name : undefined
      };
    }
  };
}
...
function normalizeShareId(id = '') {
  ...
  if (/^[a-f0-9]{16,64}$/.test(legacy)) return legacy;
  if (/^[a-zA-Z0-9_-]{2,48}$/.test(raw)) return raw;
  return '';
}

const RESERVED_SHARE_SLUGS = new Set([
  'login','logout','admin','api','shared','share','s','root','me','assets','static',
  'favicon','robots','sitemap','index','about','help','support','settings','account',
  'dashboard','drive','cloud','upload','download','new','create','test','demo','_next'
]);

async function resolveShare(env, id) {
  const clean = normalizeShareId(id);
  if (!clean) return null;
  const exact = await getShare(env, clean);
  if (exact) return exact;
  return getShare(env, clean.toLowerCase());
}
...
async function listShares(env, filterPath = '') {
  ...
  if (!keys.length) return [];
  const records = await batchGetKvJson(env, keys);
  const shares = keys
    .map(key => normalizeShareRecord(records.get(key)))
    .filter(Boolean);
...
async function createShare(env, body = {}) {
  ...
  const customSlug = String(body.customId || body.slug || '').trim();
  let id;
  if (customSlug) {
    id = validateShareSlug(customSlug);
    if (await getShare(env, id)) throw new Error('该短链后缀已被占用，请换一个');
  } else {
...
function validateShareSlug(value) {
  const slug = String(value || '').trim();
  if (!/^[a-zA-Z0-9_-]{2,48}$/.test(slug)) throw new Error('短链后缀仅支持 2-48 位字母、数字、下划线或中划线');
  if (RESERVED_SHARE_SLUGS.has(slug.toLowerCase())) throw new Error('该后缀为系统保留词，请换一个');
  return slug;
}
...
async function updateShare(env, id, body = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(body, 'customId')) {
    const slug = validateShareSlug(body.customId);
    if (slug !== share.id) {
      if (await getShare(env, slug)) throw new Error('该短链后缀已被占用，请换一个');
      updated.id = slug;
    }
  }
  ...
  if (updated.id !== share.id) await deleteShare(env, share.id);
  await saveShare(env, updated);
  return getShare(env, updated.id);
}

async function refreshShareLink(env, id, body = {}) {
  const share = await resolveShare(env, id);
  if (!share) throw new Error('not found');
  const customSlug = String(body.customId || '').trim();
  const now = new Date().toISOString();
  let newId = crypto.randomUUID().replace(/-/g, '');
  if (customSlug) {
    newId = validateShareSlug(customSlug);
    if (newId !== share.id && await getShare(env, newId)) throw new Error('该短链后缀已被占用，请换一个');
  } else if (!/^[a-f0-9]{32}$/.test(share.id)) {
    // Keep custom slug — regenerating only resets counters / timestamps
    newId = share.id;
  }
  const refreshed = {
    ...share,
  ...
  // Only remove the old record when the id actually changed
  if (newId !== share.id) await deleteShare(env, share.id);
  return getShare(env, refreshed.id);
}

async function checkShareSlugAvailability(env, slug) {
  const candidate = validateShareSlug(slug);
  const taken = !!(await getShare(env, candidate));
  return { ok: !taken, available: !taken, slug: candidate };
}
...
async function consumeShareAccess(env, share) {
  // Re-read to reduce lost-update races under concurrent unlocks
  const fresh = (share?.id ? await getShare(env, share.id) : null) || share;
  const reason = shareInactiveReason(fresh);
  if (reason) throw new Error(reason);
  const updated = {
    ...fresh,
    accessCount: Math.max(0, Number(fresh.accessCount || 0)) + 1,
    updatedAt: new Date().toISOString()
  };
  await saveShare(env, updated);
...
async function mutateDirectoryIndex(env, path, mutator) {
  const clean = assertVirtualPath(path, { allowRoot: true });
  // Retry read-modify-write to reduce lost updates under concurrent folder edits
  let lastError = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      await putDirectoryIndex(env, clean, normalizeDirectoryIndex(index, clean));
      return;
    } catch (err) {
      lastError = err;
      if (attempt === 2) break;
...
async function listDirectory(env, path = '') {
  const clean = assertVirtualPath(path, { allowRoot: true });
  const index = await getDirectoryIndex(env, clean);
  const folders = await mapWithConcurrency(index.folders, 8, async name => {
    const folderPath = joinVirtualPath(clean, name);
    const [entry, files] = await Promise.all([
      getFolderEntry(env, folderPath),
      listAllFileEntries(env, folderPath)
    ]);
    let size = 0;
    let uploaded = '';
    for (const file of files) {
      size += Number(file.size) || 0;
      const t = file.uploaded || file.updatedAt || '';
      if (t && (!uploaded || t > uploaded)) uploaded = t;
    }
    if (!uploaded) uploaded = entry?.updatedAt || entry?.createdAt || '';
    return { name, size, uploaded };
  });
  return {
    folders,
    files: index.files
...
async function findAllReferencedStorageKeys(env) {
  ...
  if (DB) {
    ...
    while (safety < 500) {
      const result = afterKey
        ? await DB.prepare(
            `SELECT "key", "value" FROM ${D1_KV_TABLE} WHERE "key" > ? AND "key" < ? AND (expires_at IS NULL OR expires_at > ?) ORDER BY "key" LIMIT ?`
          ).bind(afterKey, bound, now, PAGE).all()
        : await DB.prepare(
            `SELECT "key", "value" FROM ${D1_KV_TABLE} WHERE "key" >= ? AND "key" < ? AND (expires_at IS NULL OR expires_at > ?) ORDER BY "key" LIMIT ?`
          ).bind(prefix, bound, now, PAGE).all();
      const rows = result.results || [];
...
async function findOrphanStorageKeys(env, R2, ctx) {
  ...
  do {
    ...
    for (const obj of listed.objects) {
      r2Objects.push({ key: obj.key, size: obj.size || 0 });
    }
    cursor = listed.truncated ? listed.cursor : undefined;
    safety++;
  ...
  // Keep UI assets out of orphan cleanup
...
async function deleteOrphanStorageKeys(env, R2, keys, ctx) {
  if (!keys || !keys.length) return { deleted: 0, failed: 0, freedBytes: 0, skipped: 0 };

  // Re-scan references so clients cannot delete live objects via a stale orphan list
  const fresh = await findOrphanStorageKeys(env, R2, ctx);
  const orphanSet = new Set((fresh.orphans || []).map(o => o.key));
  const keysToDelete = [...new Set(keys.filter(Boolean).filter(k => orphanSet.has(k)))];
  const skipped = keys.length - keysToDelete.length;
  if (!keysToDelete.length) return { deleted: 0, failed: 0, freedBytes: 0, skipped };
  ...
  // Invalidate usage cache after bulk deletes
  if (deleted > 0 && R2) invalidateStorageUsageCaches(R2);

  return { deleted, failed, freedBytes, skipped };
}
  ...
  const usages = await Promise.all(nodes.map(async (node, index) => {
    try {
      if (node.id === MAIN_STORAGE_NODE_ID || node.storageType === 'r2') {
        // Prefer cached / estimated usage; full R2 list is expensive
        const estimatedUsed = await getStoredNodeUsage(env, MAIN_STORAGE_NODE_ID).catch(() => 0);
        let r2Used = estimatedUsed;
        try {
...
function isNodeRequestAuthorized(request, env) {
  const expectedToken = env.STORAGE_NODE_TOKEN || env.ACCESS_PASSWORD;
...
function isAllowedNodePartKey(key = '') {
  const value = String(key || '');
  return value.startsWith(NODE_PART_PREFIX) && !value.includes('..') && !value.includes('\\');
}
...
function distributedPartReceiptKey(sessionId, partNumber) {
  return MULTIPART_SESSION_PREFIX + String(sessionId || '') + ':ok:' + String(partNumber || '');
}

async function markDistributedPartUploaded(env, sessionId, partNumber, size = 0) {
  await kvPutRaw(env, distributedPartReceiptKey(sessionId, partNumber), JSON.stringify({
    partNumber,
...
async function listDistributedPartReceipts(env, sessionId, parts) {
  const list = Array.isArray(parts) ? parts : [];
  const receipts = new Map();
  await mapWithConcurrency(list, 16, async (part) => {
    const raw = await kvGetRaw(env, distributedPartReceiptKey(sessionId, part.partNumber));
    if (!raw) return;
    const data = safeJsonParse(raw);
    if (data) receipts.set(part.partNumber, data);
  });
  return receipts;
}

async function clearDistributedPartReceipts(env, sessionId, parts) {
  const list = Array.isArray(parts) ? parts : [];
  await mapWithConcurrency(list, 16, part =>
    kvDelete(env, distributedPartReceiptKey(sessionId, part.partNumber)).catch(() => {})
  );
}

async function verifyDistributedSessionParts(env, R2, session) {
  const parts = Array.isArray(session?.parts) ? session.parts : [];
  if (!parts.length) throw new Error('session has no parts');

  const receipts = await listDistributedPartReceipts(env, session.sessionId, parts);
  const unverified = parts.filter(part => !receipts.has(part.partNumber));

  // 上传接口已写入回执的分片视为已就绪，避免完成阶段对上百个节点做 HEAD 导致超时
  if (!unverified.length) return true;

  const nodes = await getStorageNodes(env, true);
  const nodeMap = new Map(nodes.map(n => [n.id, n]));

  const headWithRetry = async (fn) => {
    let lastErr;
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        return await fn();
    ...
    throw lastErr || new Error('part verify failed');
  };

  await mapWithConcurrency(unverified, 8, async (part) => {
    const expectedSize = Math.max(0, Number(part.size || 0));
    const isMain = part.storageType === 'r2' || part.nodeId === MAIN_STORAGE_NODE_ID;
    if (isMain) {
      await headWithRetry(async () => {
        const meta = await R2.head(part.key);
        if (!meta) throw new Error('分片 ' + part.partNumber + ' 在主存储上不存在');
        if (expectedSize > 0 && Number(meta.size || 0) !== expectedSize) {
          throw new Error('分片 ' + part.partNumber + ' 主存储大小不匹配');
        }
      });
      await markDistributedPartUploaded(env, session.sessionId, part.partNumber, expectedSize);
      return true;
    }

    const node = nodeMap.get(part.nodeId) || (part.nodeUrl && part.token
      ? { id: part.nodeId, url: part.nodeUrl, token: part.token }
      : null);
    if (!node?.url || !node?.token) {
      throw new Error('分片 ' + part.partNumber + ' 节点不可用');
    }
    await headWithRetry(async () => {
      const res = await fetch(
        String(node.url).replace(/\/+$/, '') + '/api/node/part?key=' + encodeURIComponent(part.key),
        { method: 'HEAD', headers: getNodeAuthHeaders(node) }
      ).catch(() => null);
      if (!res || !res.ok) {
        throw new Error('分片 ' + part.partNumber + ' 在节点上不存在或无法访问');
      }
      const len = Number(res.headers.get('Content-Length') || 0);
      if (expectedSize > 0 && len > 0 && len !== expectedSize) {
        throw new Error('分片 ' + part.partNumber + ' 节点大小不匹配');
      }
    });
    await markDistributedPartUploaded(env, session.sessionId, part.partNumber, expectedSize);
    return true;
...
async function handleStorageNodeApi(request, env) {
  ...
  if (url.pathname === '/api/node/r2-list' && request.method === 'GET') {
    ...
    const listed = await R2.list({ cursor, limit });
  ...
  if (!key || !isAllowedNodePartKey(key)) {
    return new Response('Invalid or missing key', { status: 400, headers: nodeCorsHeaders() });
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/api/login' && request.method === 'POST') {
      ...
      if (!await verifyAccessPassword(password, env.ACCESS_PASSWORD)) {
    ...
    if (path === '/shared' || path === '/shared/') {
      ...
      return htmlResponse(renderSharedPage(folders, files, subPath, siteTitle, cloudIconUrl));
    }
    ...
    if (path === '/api/shared-list') {
      ...
      return Response.json({ folders, files });
        ...
        // ── Share routes (clipboard API below still requires auth) ──
    if (path.startsWith('/s/') && request.method === 'GET') {
      const shareId = normalizeShareId(path.slice(3).split('/')[0] || '');
      const share = await resolveShare(env, shareId);
      if (!share) {
      ...
      const activeShare = shareNeedsPassword(share)
        ? share
        : (share.targetType === 'file' ? share : await consumeShareAccess(env, share));
      if (activeShare.targetType === 'file') {
        ...
        return htmlResponse(renderConditionalSharePage(activeShare, { file }, siteTitle, cloudIconUrl));
      }

      const subPath = assertVirtualPath(url.searchParams.get('path') || '', { allowRoot: true });
      const folderPath = shareTargetPath(activeShare, subPath);
      const { folders, files } = await listDirectory(env, folderPath);
      return htmlResponse(renderConditionalSharePage(activeShare, { folders, files, currentPath: subPath }, siteTitle, cloudIconUrl));
    }

    if (path === '/api/share-access' && request.method === 'POST') {
      if (!isSameOriginRequest(request)) return csrfErrorResponse();
      const body = await request.json().catch(() => ({}));
      const share = await resolveShare(env, body.id);
    ...
    if (path === '/api/share-download' && (request.method === 'GET' || request.method === 'HEAD')) {
      const share = await resolveShare(env, url.searchParams.get('id') || '');
      ...
      // Password shares may already be at maxAccesses after unlock; cookie grants download.
      // Open shares must still enforce the limit on every download.
      if (reason === 'access_limit' && !(shareNeedsPassword(share) && hasAccess)) {
        return new Response('Share access limit reached', { status: 410 });
      }
      if (shareNeedsPassword(share) && !hasAccess) return new Response('Share password required', { status: 403 });

      // Count each open-share download so /api/share-download cannot bypass maxAccesses
      if (!shareNeedsPassword(share) && request.method === 'GET') {
        try {
          await consumeShareAccess(env, share);
        } catch (err) {
          const msg = err?.message || '';
          if (msg === 'access_limit' || msg === 'expired') {
            return new Response(msg === 'access_limit' ? 'Share access limit reached' : 'Share expired', { status: 410 });
          }
          throw err;
        }
      }
    ...
    if (!authed) {
      if (path.startsWith('/api/')) {
        ...
        if (path === '/api/download') {
          ...
          let cleanShared = '';
          try {
            cleanShared = assertVirtualPath(filePath);
          } catch {
            return new Response('Invalid path', { status: 400 });
          }
          if (cleanShared === SHARED_PREFIX || cleanShared.startsWith(SHARED_PREFIX + '/')) {
            return storedVirtualFileResponse(request, R2, cleanShared, env, {
              filename: virtualPathName(cleanShared),
              contentType: getMimeType(cleanShared)
            });
          }
        }
    ...
    // UI settings: custom drive background
    if (path === '/api/ui-settings' && request.method === 'GET') {
      const settings = await kvGetJson(env, UI_SETTINGS_KV_KEY) || {};
      const hasBg = !!settings.driveBackground;
      return jsonResponse({
        driveBackgroundUrl: hasBg ? '/api/ui-background' : '',
    ...
    if (path === '/api/ui-background' && request.method === 'PUT') {
      ...
      if (buf.byteLength > 8 * 1024 * 1024) {
        return jsonResponse({ ok: false, error: 'image too large (max 8MB)' }, 413);
      }
      await R2.put(UI_DRIVE_BG_R2_KEY, buf, {
        httpMetadata: { contentType: contentType.split(';')[0].trim() || 'image/jpeg' }
      });
      await kvPutJson(env, UI_SETTINGS_KV_KEY, {
        driveBackground: true,
        updatedAt: new Date().toISOString()
      });
      return jsonResponse({ ok: true, url: '/api/ui-background' });
    }
    ...
    if (path === '/api/shares/refresh' && request.method === 'POST') {
      try {
        ...
        const share = await refreshShareLink(env, id, body);
    ...
    if (path === '/api/shares/slug-check' && request.method === 'GET') {
      try {
        const result = await checkShareSlugAvailability(env, url.searchParams.get('slug') || '');
        return jsonResponse(result);
      } catch (err) {
        return jsonResponse({ ok: false, available: false, error: err?.message || 'invalid slug' }, 400);
      }
    }
    ...
    if (path === '/api/shares' && request.method === 'PUT') {
      try {
        const body = await request.json().catch(() => ({}));
        const id = body.id || url.searchParams.get('id') || '';
        const existing = await getShare(env, id);
        if (!existing) throw new Error('not found');
        const share = await updateShare(env, existing.id, body);
        return jsonResponse({ ok: true, share: publicShare(share) });
      } catch (err) {
    ...
    if (path === '/api/shares' && request.method === 'DELETE') {
      ...
      const existing = await resolveShare(env, id);
      if (!existing) return jsonResponse({ ok: false, error: 'not found' }, 404);
      await deleteShare(env, existing.id);
      return jsonResponse({ ok: true });
    }
    ...
    if (path === '/api/list') {
      ...
      return Response.json({ folders, files });
        ...
        // Storage usage — headline "已用" uses logical FS sizes (matches file/folder UI).
        // Per-node details use live R2 usage like old.js (no inflated KV estimates).
        if (path === '/api/storage') {
          const nodes = await getStorageNodes(env);
          const logicalUsed = await calculateLogicalStorageUsage(env).catch(() => 0);
          let mainUsed = 0;
          try {
            mainUsed = await calculateR2Usage(R2);
          } catch {
            mainUsed = await getStoredNodeUsage(env, MAIN_STORAGE_NODE_ID).catch(() => 0);
          }
          const usageNodes = [{
            id: 'main',
            name: '主控账号',
          ...
          const physicalUsed = usageNodes.reduce((sum, node) => sum + (node.used || 0), 0);
    ...
    if (path === '/api/upload' && request.method === 'POST') {
      ...
      if (R2) invalidateStorageUsageCaches(R2);
      return Response.json({ ok: true });
    }
    ...
    if (path === '/api/multipart/part' && request.method === 'POST') {
      ...
      if (!session?.path || !session?.storageKey) return new Response('Invalid multipart session', { status: 400 });
    ...
    if (path === '/api/distributed/main-part' && request.method === 'PUT') {
      ...
      const session = safeJsonParse(raw);
      if (!session) return jsonResponse({ ok: false, error: 'invalid session' }, 400);
      ...
      await markDistributedPartUploaded(env, sessionId, partNumber, expectedSize || contentLength);
    ...
    if (path === '/api/distributed/complete' && request.method === 'POST') {
      ...
      if (!raw) return jsonResponse({ ok: false, error: 'session expired' }, 404);
      const session = safeJsonParse(raw);
      if (!session?.path || !session?.storageKey || !Array.isArray(session.parts)) {
        return jsonResponse({ ok: false, error: 'invalid session' }, 400);
      }
      try {
        await verifyDistributedSessionParts(env, R2, session);
      } catch (err) {
        return jsonResponse({ ok: false, error: err?.message || 'parts incomplete' }, 400);
      }
      const manifest = {
      ...
      if (R2) invalidateStorageUsageCaches(R2);
      await clearDistributedPartReceipts(env, sessionId, session.parts);
    ...
    if (path === '/api/clipboard/paste' && request.method === 'POST') {
      ...
      const failed = results.filter(item => !item.ok);
      invalidateStorageUsageCaches(R2);
      return jsonResponse({
        ok: failed.length === 0,
        action,
        sourcePath,
    ...
    if (path === '/' || path === '') {
      ...
      return htmlResponse(renderDrivePage(folders, files, cleanPrefix, siteTitle, cloudIconUrl));
    }
