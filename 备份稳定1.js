...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header.app-bar, .layout, footer.foot-bar, .login-wrap { position: relative; z-index: 1; }
...
function renderDrivePage(folders, files, currentPath, siteTitle, cloudIconUrl = '') {
  ...
  const breadcrumb = `<nav class="breadcrumb">
    ...
    return `<div class="file-card" data-name="${escapeAttr(name)}"
...
async function deleteOrphanStorageKeys(env, R2, keys, ctx) {
  ...
  if (deleted > 0 && R2) r2UsageCache.delete(R2);
...
async function verifyDistributedSessionParts(env, R2, session) {
  ...
  const checks = await mapWithConcurrency(parts, 4, async (part) => {
    ...
    if (isMain) {
      ...
      if (!meta) throw new Error('part ' + part.partNumber + ' missing on main storage');
      if (expectedSize > 0 && Number(meta.size || 0) !== expectedSize) {
        throw new Error('part ' + part.partNumber + ' size mismatch on main storage');
      }
      return true;
    }
    ...
    if (!node?.url || !node?.token) {
      throw new Error('part ' + part.partNumber + ' node unavailable');
    }
    const res = await fetch(
    ...
    if (!res || !res.ok) {
      throw new Error('part ' + part.partNumber + ' missing on node');
    }
    const len = Number(res.headers.get('Content-Length') || 0);
    if (expectedSize > 0 && len > 0 && len !== expectedSize) {
      throw new Error('part ' + part.partNumber + ' size mismatch on node');
    }
    return true;
  });

  return checks.length === parts.length;
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/api/list') {
      ...
      return Response.json({ folders, files });
        ...
        // Storage usage (for capacity display) — uses cached R2 usage to avoid full-bucket scans
        if (path === '/api/storage') {
          const nodes = await getStorageNodes(env);
          const estimatedMain = await getStoredNodeUsage(env, MAIN_STORAGE_NODE_ID).catch(() => 0);
          let mainUsed = estimatedMain;
          try {
            mainUsed = Math.max(estimatedMain, await calculateR2Usage(R2));
          } catch {
            // keep estimated
          }
          const usageNodes = [{
          ...
          const nodeUsages = await Promise.all(nodes.map(async node => {
            try {
              ...
              const data = await res.json();
              const estimated = await getStoredNodeUsage(env, node.id).catch(() => 0);
              return {
                id: node.id,
                name: node.name,
                used: Math.max(Number(data.used || 0), estimated),
